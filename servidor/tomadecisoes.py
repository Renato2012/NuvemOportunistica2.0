...
def verLog(nomeThread, delay):
    ...
    while True:
        ...
        if result == 0: 	# achou mensagem, chama scprit liga-desliga.sh
            ...
            os.system("bash ~/cliente/liga-desliga.sh l " + str(qtd))
...
def estado(cliente, msgRecebida):
    ...
    if str(msgRecebida) == "stop":
# Antes de desabilitar o host, ver se tem VMs, para migra-las para as MFs.
        result = commands.getoutput("onevm list -l STAT,HOST | grep "+ nome_host)
        if "runn" in str(result):		# tem VM em estado runn na MF        
